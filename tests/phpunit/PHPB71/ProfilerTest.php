...
namespace Rundiz\Profiler\Tests\PHPB71;
...
class ProfilerTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function setUp()
    {
        ...
        // build log data
        $this->Profiler->Console->log('info', ['Test' => 'Test log array data', 'Values' => ['Name' => 'Vee', 'Last' => 'W']]);
        $this->Profiler->Console->log('debug', 'Debug log or normal log data.');
        $this->Profiler->Console->log('info', ['Test' => 'Test log array data', 'Values' => 'Only have 1 level.']);
        ...
        $this->Profiler->Console->timeload('Time taken to this line '.__FILE__.': '.__LINE__, '', '', 'phpunit_timeload_test');
        usleep(100000);
        $this->Profiler->Console->timeload('After sleep. Time taken to this line '.__FILE__.': '.__LINE__, '', '', 'phpunit_timeload_test');

        // build memory usage data
        $this->Profiler->Console->memoryUsage('Before loop memory usage at file:'.__FILE__.': '.__LINE__, '', '', 'phpunit_memoryusage_test');
        $data = '';
        ...
        $this->Profiler->Console->memoryUsage('After loop memory usage at file:'.__FILE__.': '.__LINE__, '', '', 'phpunit_memoryusage_test');
        unset($data, $long_str);
    }// setUp.
    ...
    public function testGetProfilerDumpData()
    {
        ...
        if (is_array($dump_results) && array_key_exists('log_sections', $dump_results)) {
            foreach ($dump_results['log_sections'] as $section => $section_items) {
                $re_formatted_profiler_results[$section] = [];
        ...
        // assert count values
        $this->assertCount(9, $re_formatted_profiler_results['Logs']);
        $this->assertCount(2, $re_formatted_profiler_results['Time Load']);
        $this->assertCount(5, $re_formatted_profiler_results['Memory Usage']);
        $this->assertGreaterThanOrEqual(3, $re_formatted_profiler_results['Files']);

        unset($re_formatted_profiler_results);

        // assert count log type
        $this->assertEquals(2, $this->Profiler->countTotalLogType('info'));

        // assert not empty match key
        $this->assertNotEmpty($this->Profiler->summaryMatchKey('Time Load', 'phpunit_timeload_test', 1));
        $this->assertNotEmpty($this->Profiler->summaryMatchKey('Memory Usage', 'phpunit_memoryusage_test', 4));
